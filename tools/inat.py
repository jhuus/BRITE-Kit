...
class Main:
    def __init__(self, species_name, output_path, max_downloads, require_photos, min_rec_number):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='', help='Species name (scientific name is most reliable).')
    parser.add_argument('--photo', default=False, action='store_true', help='Optional flag to only download from observations with photos, to ensure correct ID.')
    parser.add_argument('--rec_min', type=int, default=None, help='If specified, ignore recording numbers below this. Default = None.')

    args = parser.parse_args()

    Main(args.name, args.dir, args.max, args.photo, args.rec_min).run()
