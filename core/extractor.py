...
class Extractor:
    def __init__(self, audio_path, db_name, species_name, species_code):
        self.audio_path = audio_path # path to folder containing recordings to extract from
        self.db = self._get_database(db_name)
        self.source = ''    # keep it simple - don't use the Source table from HawkEars
        self.category = ''  # keep it simple - don't use the Category table from HawkEars
    ...
    def insert_spectrograms(self, recording_path, offsets):
        ...
        num_inserted = 0
        specs = self.audio.get_spectrograms(offsets, segment_len=cfg.audio.segment_len)
