...
class MainModel(LightningModule):
    def __init__(self, train_class_names, train_class_codes, test_class_names, weights, model_name, num_train_specs=0):
        super().__init__()
        ...
        self.base_model = self._create_model(model_name)
    ...
    def _create_model(self, model_name):
        ...
        # create the model
        if model_name.startswith('custom_efficientnet'):
        ...
        while start_idx < len(specs):
            ...
            with torch.no_grad():
                ...
                if use_softmax:
                    ...
                    block_predictions = torch.sigmoid(block_predictions)

                if predictions is None:
                    predictions = block_predictions.cpu().numpy()
                else:
                    predictions = np.concatenate((predictions, block_predictions.cpu().numpy()))
